...
<body>
    <div class="container">
        ...
        <div id="search">
            ...
            <form action="edit-project.php?id=<?php echo $projectId ?>&status=<?php echo $status ?>" method="post">
                            ...
                            if(sqlsrv_has_rows($result) > 0){
                                ...
                                echo '<tbody>
                                        <tr>
                                            ...
                                            <td>'. $row['Email'] .'</td>
                                            <td>'. $row['Phone'] .'</td>
                                            <td><a href="add-intern-project.php?id='.$row['InternID'].'&pid='.$projectId.'&status='.$status.'"><button id="add">Add</button></a></td>
                                        </tr>
                                      </tbody>';
                                    }
                                 } else {
...
<?php
    if(isset($_POST['change'])){
        ...
        header("Location: projects-view.php?id=$projectId&status=$status");
    }
    sqlsrv_close($conn);
?>